...
namespace RoundedTask
{
    ...
    internal sealed class SettingsForm : Form
    {
        ...
        private const int DesignHeight = 930;
        private const int MinClientHeight = 360;
        private const int WindowScreenMargin = 48;
        ...
        private readonly ModernScrollPanel _scrollHost;
        ...
        public SettingsForm(AppSettings settings)
        {
            ...
            MaximizeBox = false;
            MinimizeBox = false;
            ClientSize = GetFittedClientSize();
            Font = new Font("Segoe UI", 9F);
            BackColor = Palette.Page;
            Icon = LoadAppIcon();

            _scrollHost = new ModernScrollPanel();
            _scrollHost.Dock = DockStyle.Fill;
            _scrollHost.ContentSize = new Size(DesignWidth, DesignHeight);
            Controls.Add(_scrollHost);
            MouseWheel += delegate(object sender, MouseEventArgs e) { _scrollHost.ScrollFromExternalWheel(e); };

            Control page = _scrollHost.ContentPanel;
            ...
            page.Controls.Add(behaviorPanel);
            ...
            UpdateControlStates();
            _scrollHost.HookScrollableChildren();
            _scrollHost.RefreshScrollState();
            PlaceInsideWorkingArea();
        }
        ...
        private Size GetFittedClientSize()
        {
            Rectangle workingArea = GetCurrentWorkingArea();
            int nonClientHeight = Math.Max(0, Size.Height - ClientSize.Height);
            int availableHeight = workingArea.Height - WindowScreenMargin - nonClientHeight;
            int height = Math.Min(DesignHeight, Math.Max(MinClientHeight, availableHeight));
        ...
        private void PlaceInsideWorkingArea()
        {
            ...
            {
                x = Math.Max(workingArea.Left, workingArea.Right - Width);
            }

            if (y + Height > workingArea.Bottom)
            {
                y = Math.Max(workingArea.Top, workingArea.Bottom - Height);
            }

            Location = new Point(x, y);
        }

        private static Rectangle GetCurrentWorkingArea()
        {
            try
            {
        ...
        private sealed class ModernScrollPanel : Control
        {
            private const int ScrollbarGutter = 14;
            private const int ScrollbarInset = 10;
            private const int ScrollStep = 56;
            private const int FocusMargin = 18;
            ...
            public ModernScrollPanel()
            {
                ...
                _contentPanel.BackColor = Palette.Page;
                _contentPanel.Location = Point.Empty;
                _contentPanel.Size = new Size(DesignWidth, DesignHeight);
                Controls.Add(_contentPanel);

                _contentSize = _contentPanel.Size;
            }
            ...
            public void RefreshScrollState()
            {
                ...
                int gutter = NeedsScroll ? ScrollbarGutter : 0;
                _contentPanel.Bounds = new Rectangle(0, -_scrollOffset, Math.Max(0, Width - gutter), _contentSize.Height);
                Invalidate();
            }

            public void HookScrollableChildren()
            {
                HookScrollableChildren(_contentPanel);
            }

            public void ScrollFromExternalWheel(MouseEventArgs e)
            {
                ScrollFromWheel(e);
            }
            ...
            protected override void OnMouseDown(MouseEventArgs e)
            {
                ...
                if (!NeedsScroll || e.Button != MouseButtons.Left)
                {
                    return;
                }
                ...
                {
                    ScrollBy(e.Y < thumb.Top ? -Height : Height);
                }
            }
            ...
            protected override void OnMouseMove(MouseEventArgs e)
            {
                ...
                if (_draggingThumb)
                {
                    ...
                    Rectangle thumb = ThumbRect;
                    int range = Math.Max(1, track.Height - thumb.Height);
                    int top = e.Y - _dragOffset - track.Top;
                    SetScrollOffset((int)Math.Round((double)top * MaxScroll / range));
                    return;
                }

                bool nextHot = ThumbRect.Contains(e.Location);
                if (_thumbHot != nextHot)
                {
                    _thumbHot = nextHot;
                    Invalidate();
                }
            ...
            protected override void OnPaint(PaintEventArgs e)
            {
                ...
                using (GraphicsPath trackPath = CreateRoundRect(track, 4))
                using (SolidBrush trackBrush = new SolidBrush(Color.FromArgb(224, 234, 247)))
                {
                    e.Graphics.FillPath(trackBrush, trackPath);
                }

                Color top = _thumbHot || _draggingThumb ? Palette.LightBlue : Palette.Blue;
                Color bottom = _draggingThumb ? Palette.StrongBlue : Color.FromArgb(18, 108, 248);
                using (GraphicsPath thumbPath = CreateRoundRect(thumb, 5))
                using (LinearGradientBrush thumbBrush = new LinearGradientBrush(thumb, top, bottom, LinearGradientMode.Vertical))
                {
                    e.Graphics.FillPath(thumbBrush, thumbPath);
                }
            }

            private void HookScrollableChildren(Control parent)
            {
                parent.MouseWheel -= OnChildMouseWheel;
                parent.MouseWheel += OnChildMouseWheel;
                parent.Enter -= OnChildEnter;
                parent.Enter += OnChildEnter;

                foreach (Control child in parent.Controls)
                {
                    HookScrollableChildren(child);
                }
            }
            ...
            private void OnChildEnter(object sender, EventArgs e)
            {
                Control source = sender as Control;
                if (source != null)
                {
                    ScrollChildIntoView(source);
                }
            }

            private void ScrollFromWheel(MouseEventArgs e)
            {
                if (!NeedsScroll)
                {
                    return;
                ...
                int clicks = Math.Max(1, Math.Abs(e.Delta) / 120);
                int direction = e.Delta > 0 ? -1 : 1;
                ScrollBy(direction * ScrollStep * clicks);
            }

            private void ScrollChildIntoView(Control control)
            {
                if (!NeedsScroll || control == null || !control.Visible)
                {
                    return;
                }

                Rectangle bounds = GetBoundsInContent(control);
                int viewportTop = _scrollOffset;
                int viewportBottom = _scrollOffset + Height;

                if (bounds.Top - FocusMargin < viewportTop)
                {
                    SetScrollOffset(bounds.Top - FocusMargin);
                    return;
                }

                if (bounds.Bottom + FocusMargin > viewportBottom)
                {
                    SetScrollOffset(bounds.Bottom + FocusMargin - Height);
                }
            }

            private Rectangle GetBoundsInContent(Control control)
            {
                Rectangle bounds = control.Bounds;
                Control parent = control.Parent;

                while (parent != null && parent != _contentPanel)
                {
            ...
            private static bool IsInsideNumberBox(Control control)
            {
                while (control != null)
                {
                    if (control is ModernNumberBox)
                    {
                        return true;
                    }
            ...
            {
                get { return Math.Max(0, _contentSize.Height - Height); }
            }
            ...
            private Rectangle TrackRect
            {
                get
                {
                    int height = Math.Max(0, Height - (ScrollbarInset * 2));
                    return new Rectangle(Width - 10, ScrollbarInset, 5, height);
                }
            }
            ...
            private Rectangle ThumbRect
            {
                get
                {
                    ...
                    Rectangle track = TrackRect;
                    int height = Math.Max(44, (int)Math.Round((double)Height * track.Height / _contentSize.Height));
                    height = Math.Min(track.Height, height);
                    int range = Math.Max(1, track.Height - height);
                    int top = track.Top + (int)Math.Round((double)_scrollOffset * range / MaxScroll);
                    return new Rectangle(track.Left - 1, top, track.Width + 2, height);
                }
            }
        }
