...
namespace RoundedTask
{
    internal sealed class TaskbarStyler
    {
        ...
        private bool ApplyToWindow(TaskbarInfo taskbar, bool force)
        {
            ...
            StabilizeDynamicMeasurements(taskbar);
        ...
        private void StabilizeDynamicMeasurements(TaskbarInfo taskbar)
        {
            if (!String.Equals(_settings.ShapeMode, "DynamicSegments", StringComparison.OrdinalIgnoreCase))
            {
            ...
            if (taskbar.AppListRect.IsEmpty)
            {
                ...
                if (_dynamicMeasurements.TryGetValue(taskbar.Hwnd, out missingState) && missingState.HasLast && missingState.MissingReads < 3)
                {
            ...
            int suspiciousShrink = Math.Max(96, previousWidth / 4);

            if (previousWidth > 0 && currentWidth > 0 && previousWidth - currentWidth >= suspiciousShrink)
            {
                state.ShrinkReads++;
                if (state.ShrinkReads < 3)
                {
